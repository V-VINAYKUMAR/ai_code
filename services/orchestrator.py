...
class Orchestrator:
    ...
    def _clean_code(code: str) -> str:
        """Remove Markdown code fences before code is executed."""
        code = code.strip()

        if code.startswith("```"):
            lines = code.splitlines()

            # Remove opening fence such as ```python
        ...
        for group_num in sorted(groups.keys()):
            ...
            if len(group_steps) == 1:
                ...
                step_result = await self._execute_step(
                    group_steps[0], context_files, results, memory_context,
                    latest_code, generated_files, callback
                )
                if step_result.get("code"):
                    latest_code = step_result["code"]
                results.append(step_result)
            else:
                # Code-producing agents share latest_code, so they must run
                # sequentially. Only independent tool steps should run in parallel.
                code_agents = {"coder", "tester", "debugger"}

                if any(s.get("agent") in code_agents for s in group_steps):
                    await self._notify(
                        callback,
                        f"⚙️ Running {len(group_steps)} dependent steps sequentially…\n"
                    )
                ...
                # Only independent non-code steps run in parallel.
                await self._notify(
                    callback,
                    f"⚡ Running {len(group_steps)} steps in parallel…\n"
                )
    ...
    async def _execute_step(
        ...
        if agent == "coder":
            ...
            # Prefer generated in-memory file content before reading disk.
            existing = generated_files.get(
                file_path,
        ...
        # Tester agent
            ...
            # Run the test file generated by the Coder when available.
            ...
            source_code = "\n\n".join(source_parts) or latest_code
            ...
            if generated_test_files:
                ...
                return {
                    ...
                    "type": "test",
                    "code": test_code,
                    "debug": "tests-failed" if test_failed else "tests-passed",
                }

            # Fallback when no generated test file exists.
